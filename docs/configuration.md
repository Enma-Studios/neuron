...
Searches run on every enabled engine through `:neuron, :search, :engines`. **The default is DuckDuckGo and Brave**: a keyword engine that returns public results to a signed-out cloud browser, plus a keyed API that runs only when it has a key. `Neuron.Search.Yandex` is supported but off by default, because it consent-walls cloud browsers on every query; enable it explicitly with `engines:` once those walls are handled. The model tailors each query to its platform before anything runs: LinkedIn, X, and Reddit get native, operator-free queries, while the keyword engines keep operators such as `site:` and quoted phrases, aimed at pages a prospective buyer publishes itself.
