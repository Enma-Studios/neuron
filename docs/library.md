...
`run.result.status` is `:target_met`, `:partial`, or `:no_qualified_leads`. Results include `campaign_id`, `campaign_run_id`, `target_count`, `returned_count`, `leads`, `summary`, `stop_reason`, `failures`, `selection`, `rejected_sources`, `role_signals`, and `people_pages`.
