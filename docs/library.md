...
`Neuron.Campaign.run/2` starts and awaits the durable campaign pipeline and returns the standard run snapshot. The normalized brief separates `seller_profile` (our domain, offering, field, geography) from `target_profile` (prospective markets, roles, geography, exclusions). `seller_profile.domain` (and the campaign's `domain`) is the registrable domain of the input `url`, `website` or `source_url`: `https://careers.booking.com/...` gives `booking.com`. An extracted or supplied `domain` field is used only when there is no URL and it is itself a hostname; a campaign with neither returns `needs_input` with reason `:organization_domain_required`. People whose employer is the seller's domain are never leads (`rejected_by.seller`). Keep its `campaign_id` when requesting new executions of the same campaign. Each execution and ingestion child has its own UUID.
...
`run.result.status` is `:target_met`, `:partial`, or `:no_qualified_leads`. Results include `campaign_id`, `campaign_run_id`, `target_count`, `returned_count`, `leads`, `summary`, `stop_reason`, `failures`, `selection`, `rejected_sources`, `role_signals`, `people_pages`, and `captures`.

`captures` holds the retained page behind every claim and contact channel in `leads`, once each: `%{url, retrieved_at, content_hash, markdown}`. `markdown` is the exact Markdown Neuron validated each excerpt against, so every excerpt in the result is a byte-for-byte slice of its page, and `content_hash` is its lowercase hex SHA-256. Each claim in a lead's `evidence` names its page by `url` and `content_hash`, and each contact channel carries the claim it was observed from as `evidence`, so an address can be checked against the bytes of the page `evidence_urls` names. A page the run read that backs no returned claim is not included. `retrieved_at` is an ISO 8601 time at which those bytes were last read; it is `nil` for a page not read since v0.2.18, and set the next time the page is read. A caller's own assertion (`url: "urn:neuron:user"`) is behind no page and has no `content_hash`.
...
### What a host sends for the size gate and industry weight

The size gate and the industry weight do nothing unless the host sends their fields. Neuron never infers them from `target_organizations`: a sentence such as "Software companies between 20 and 300 people" there is used for search and market fit, not as a gate.
...
- **`company_size`** is a map `%{min: integer, max: integer}` in employees. Either bound may be `nil` or left out for an open range (`%{min: 20}`). String keys (`"min"`, `"max"`) are accepted; the bounds themselves must be integers. A string or any other non-integer bound is not parsed: intake returns `{:error, {:invalid_company_size, :min | :max, value}}` naming the bound and the value sent. An organization is rejected when every size it states is below `min` or above `max`: "5,000+" against `max: 300` is rejected, "51-200" against 20 to 300 passes.
- **`industries`** is a list of terms, weighed rather than gated: a term list cannot name every industry a buyer is in, and an exact-fit CTO at an insurance marketplace was once rejected because "fintech" is not in "insurance". An organization's industry or description containing any term as whole words, case-insensitively, is a `:match` (`"SaaS"` matches "A SaaS payroll platform" but not "SaaSy"). The `industry` score component is 1.0 for a match, 0.5 when the organization states no industry or description (`:unknown`), and 0.0 for a `:mismatch`, at weight 0.10. Send short terms, not sentences.
- **Unknown size passes.** An organization that states no employee count passes the size gate and is counted in `selection.unknown_by.size`. Size is only known where the organization's own pages state it.
- **Recorded per person.** Size rejections are counted in `selection.rejected_by.size`, and each rejected person appears in `selection.rejected_people` with `:size` in `checks`. Every lead and every rejected person carries `industry_match` (`:match`, `:mismatch`, `:unknown`, or `nil` when the campaign names no industries), and `selection.industry_by` counts the three over everyone considered.

`exclusions` is the only hard gate on what an organization is: it is split into one term per clause and matched against the person's facts and their employer's name, description and industry, and a match rejects the person whatever else fits.

A person must match the campaign on every check before they are scored: a sourced `name`; an observed `employer` at authority 0.8 or higher (`employer`), which is not the seller (`seller`); a title holding one of the target roles or titles as whole words (`role`); a location matching the target geography, when one is given (`geography`); and none of the exclusions in the person's facts or in their employer organization's name, description or industry (`exclusion`). The `exclusions` field is split into one term per clause on commas and semicolons, dropping a leading "not" or "no", so "Not agencies, not security vendors." becomes `["agencies", "security vendors"]`. `target_profile.company_size` (`%{min, max}` employees, either bound optional) rejects an organization whose stated `employee_count` falls outside the range (`size`); one that states no size passes and is counted in `unknown_by`. `target_profile.industries` is weighed in the score, not gated (see above). Both come from the intake fields `company_size` and `industries`; nothing is inferred. A match then needs `fit_score` at or above `selection_threshold` (default 0.5; `fit_profile.threshold` is not used by campaign ranking) (`threshold`). Campaign roles are usually categories no title contains, so `:prepare` makes one model call to expand `target_profile.roles` into the concrete job titles a team page prints, kept as `target_profile.titles`.

`selection` accounts for the latest ranking pass: `%{considered, ranked, rejected, rejected_people, role_signals, withheld_contact, rejected_by, unknown_by, industry_by}`, where `rejected_by` counts, for each check above, the candidates that failed it (one candidate can fail several). `rejected_people` lists each rejected candidate as `%{person_id, name, title, employer, industry_match, checks}`, so a rejection can be checked against the person it was made on. `considered: 0` means nobody reached the scorer; `rejected > 0` with `ranked: 0` means people were scored and rejected, and `rejected_by` names the check. It is also on `stage_data.selection` while a run is in progress, so a cancelled run keeps it. `fit_score` is written to the graph only on delivered `Lead` nodes, so a run with no leads leaves none.
